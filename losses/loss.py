...
class DetectionLoss(nn.Module):
    ...
    def forward(self, head_out, feats, targets):
        ...
        # cls: YOLOv8-style focal BCE over all anchors / num_pos, with the DENSE
        # o2m positives (~500). Focal down-weights easy background so positives
        # fire, while still penalizing hard negatives (bg that fires). Plain BCE
        # with dense pos gave bg too much weight (cls=14 flat, box frozen in v24);
        # focal with only 50 o2o pos gave bg ~0 gradient (1764/2000 fired in v22).
